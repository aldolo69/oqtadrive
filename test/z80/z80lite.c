//
// NOTE: This version differs from the original only in the explicit use of
//       float 32 literals for better comparison with Go version during test.
//
//
...
unsigned long zxsc(unsigned char* fload, unsigned char* store, int filesize, int screen) {
	...
	for (tryall_p--; tryall_p > tryall; tryall_p--) {
		...
		if (tryall_c->length.rrrr == 0) {
			costsum = 1.0f;  //literal needs 1bytes
			tryall_c++;
			// penalise literal followed by match by size of match, longer the match smaller the penalty
			if (tryall_c->length.rrrr != 0) costsum += (1.0f / (float)(tryall_c->length.rrrr)) / 10.0f;
		}
		else {
			...
			if (tryall_c->length.rrrr < 9) costsum = 2.0f;
			else costsum = 3.0f; // if length 3-8 then 2 else 3 cost
	...
	tryall_p->cost = 2.0f + (tryall_p + 1)->cost;									  
			...
			for (j = 0, i = 1; i < tryall_p->length.rrrr; i++) { // look over the full match length to see if there is a better match
				...
				if (i < MINLENGTH) {
					if (*store_c + i > 31) { // also capture if it is a control byte 255
						if ((tryall_p + i)->cost + (float)i + 1.0f < (tryall_p + j)->cost) j = i;
					}
					else if ((tryall_p + i)->cost + (float)i < (tryall_p + j)->cost) j = i;
				}
				else if (i < 9) {
					if ((tryall_p + i)->cost + 2.0f < (tryall_p + j)->cost) j = i; // add 2 to mimic storage of 3-8 match
				}
				else if ((tryall_p + i)->cost + 3.0f < (tryall_p + j)->cost) j = i; // add 3 to mimic storage of 9+ match
			}
